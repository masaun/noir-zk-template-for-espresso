...
function getInputData(input_data: any) {
  ...
  const nft_owner = BigInt(input_data.nft_data.nft_owner);
  const nft_token_id = BigInt(input_data.nft_data.nft_token_id);
  const metadata_cid_hash = BigInt(input_data.nft_data.metadata_cid_hash);
